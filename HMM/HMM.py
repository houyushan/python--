...
class HMM(object):
            ...
            for line in f:
                ...
                for k, v in enumerate(line_state):
                    ...
                    if k == 0:
                        ...
                        self.A_dic[line_state[k-1]][v] += 1 #技算转移概率
    ...
    def viterbi(self, text, states, start_p, trans_p, emit_p):
        ...
        V = [{}] # 路径图
        path = {}
        for y in states: # 初始化第一个字的各种状态的可能性
        ...
        for t in range(1, len(text)): # 每一个字
            ...
            for y in states: # 每个字的每个状态的可能
                ...
                # y0上一个字可能的状态，然后算出当前字可能的状态
                # prob则是最大可能，state是上一个字的状态
                (prob, state) = max(
            ...
            path = newpath
        if emit_p['M'].get(text[-1], 0) > emit_p['S'].get(text[-1],0): #最后一个字是词中的可能大於单独成词的可能
