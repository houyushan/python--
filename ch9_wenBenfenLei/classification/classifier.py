...
def get_data():
    '''
    获取数据
    :return: 文本数据，对应的lables
    '''
    with open("data/ham_data.txt", encoding='utf-8') as ham_f, open("data/spam_data.txt", encoding='utf8') as spam_f:
...
def remove_empty_docs(corpus, labels):
    ...
    :param corpus: 文本数据
    :param labels: 文本标签
    :return: 去空值后的文本数据列表，去空值后的文本标签列表
    '''
    filtered_corpus = []
    filtered_labels = []
...
def main():
    ...
    print('样本之一：', corpus[12])
    print('样本的label:', labels[12])
    label_name_map = ["垃圾邮件", "正常邮件"]
    print('实际类型：', label_name_map[int(labels[12])], label_name_map[int(labels[5900])])
    ...
    for document, label, predicted_label in zip(test_corpus, test_labels, svm_tfidf_predictions):
        if label == 0 and predicted_label == 0:
            print('邮件类型:', label_name_map[int(label)], int(label))
            print('预测的邮件类型:', label_name_map[int(predicted_label)], int(predicted_label))
            print('文本:-')
            print(re.sub('\n', ' ', document))
